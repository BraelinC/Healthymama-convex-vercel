...
export const generateSuggestions = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (learnedPrefs && learnedPrefs.length > 0) {
      const loves = learnedPrefs.filter(p => p.preferenceType === "food_love").map(p => p.summary);
      const dislikes = learnedPrefs.filter(p => p.preferenceType === "food_dislike").map(p => p.summary);
      const habits = learnedPrefs.filter(p => p.preferenceType === "cooking_habit").map(p => p.summary);
    ...
    if (recentMeals && recentMeals.length > 0) {
      const recentList = recentMeals.map(m => m.recipeName).join(", ");
