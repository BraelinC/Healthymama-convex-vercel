...
export const search = action({
  ...
  handler: async (ctx, args) => {
    ...
    const vectorResults = await ctx.vectorSearch("userMemories", "by_embedding", {
      vector: embedding,
      limit: topK,
      filter: (q: { eq: (field: string, value: string) => unknown }) => q.eq("userId", args.userId),
    });
    ...
    const results = memories.filter((m): m is NonNullable<typeof m> => m !== null);
...
export const listFavourites = action({
  ...
  handler: async (ctx, args) => {
    ...
    return {
      favourites: (favourites as Favourite[]).map((f: Favourite) => ({
        id: f._id,
        text: f.text,
        category: f.category,
