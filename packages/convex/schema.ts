...
export default defineSchema({
  ...
  // ========== DAYTONA SANDBOXES (OpenCode Agent Runtime) ==========

  sandboxes: defineTable({
    userId: v.string(), // Clerk user ID

    // Daytona sandbox info
    sandboxId: v.string(), // Daytona sandbox ID
    status: v.union(
      v.literal("creating"),
      v.literal("running"),
      v.literal("stopped"),
    ...
    // URLs for access
    terminalUrl: v.string(), // Web terminal URL (port 22222)
    terminalWsUrl: v.optional(v.string()), // WebSocket URL for direct connection

    // Configuration
    repoUrl: v.optional(v.string()), // Git repo cloned into sandbox
    workdir: v.string(), // Working directory

    // Runtime info
    openCodeRunning: v.boolean(), // Is OpenCode server running?
    tmuxSessionName: v.optional(v.string()), // TMUX session name
  ...
  // Sandbox command history (for debugging/replay)
  sandboxCommands: defineTable({
